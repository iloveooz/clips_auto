...
(defrule unsatisfactory-engine-state-conclutions ""
	(declare (salience 10))
...
(defrule determine-engine-state ""
	...
	(not (repair?))
=>
	(if (yes-or-no-p "БАЛАЛАЙКА!!! Does the engine start (yes/no)? ") then
		(if (yes-or-no-p "Does the engine run normally (yes/no)? ") then
			(assert (working-state engine normall))
		else
...
(defrule determine-gas-level ""
	...
	(not (repair?))
=>
	(if (not (yes-or-no-p "Does the tank have any gas in it (yes/no)? ")) then
...
(defrule determine-point-surface-state ""
	(or 
		(and (working-state engine does-not-start)
	...
	(not (repair?))
=>
	(bind ?response (ask-question "What is the surface state of the of the points (normal/burned/contaminated)? " normal burned contaminated))
...
(defrule detrmine-conductivity-test ""
	(working-state engine does-not-start)
	(spark-state engine does-not-spark)
	(charge-state battery charged)
	(not (repair?))
=>
	(if (yes-or-no-p "Is the conductivity test fir the ignition coil positive (yes/no)? ") then
...
(defrule system-banner "each new run."
	(declare (salience 10))
=>
